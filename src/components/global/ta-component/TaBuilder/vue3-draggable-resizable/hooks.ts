import { onMounted, onUnmounted, ref, watch, Ref, computed, inject } from 'vue';
import {
  getElSize,
  filterHandles,
  getId,
...
import { watchTriggerMeasureParent } from './useTriggerParentSize';
...
export function initState(props: any, emit: any) {
  ...
  return {
    ...
    primaryWidth: (val: number) => setWidth(Math.floor(val)),
    primaryHeight: (val: number) => setHeight(Math.floor(val)),
    primaryTop: (val: number) => setTop(Math.floor(val)),
    primaryLeft: (val: number) => setLeft(Math.floor(val)),
  };
}

export function initParent(containerRef: Ref<HTMLElement | undefined>) {
  const parentWidth = ref(Infinity);
  const parentHeight = ref(Infinity);
  ...
  watchTriggerMeasureParent(measureParent);
...
export function initLimitSizeAndMethods(
  ...
  const { primaryWidth, primaryHeight, primaryTop, primaryLeft } = containerProps;
  ...
  const limitMethods = {
    ...
    setTop(val: number) {
      ...
      return primaryTop(Math.min(limitProps.maxTop.value, Math.max(limitProps.minTop.value, val)));
    },
    setLeft(val: number) {
      if (props.disabledX) {
        return left.value;
      ...
      return primaryLeft(
        Math.min(limitProps.maxLeft.value, Math.max(limitProps.minLeft.value, val)),
      );
    },
  };
  return {
